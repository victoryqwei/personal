class Game {
	...
	addPassenger() {
		...
		startStation.queue.unshift(passengerId);
	...
	addBus() {
        ...
        if (stringExists(id, this.network.graph.nodes())) {
        	let colors = this.network.getColor(id);
        	let color = Object.keys(colors)[0]; // Prioritize current color line?
			this.buses.push(new Bus(nodePos.x, nodePos.y, id, color));
        }
	}
