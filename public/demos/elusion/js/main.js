...
var groundMaterial = new THREE.MeshPhongMaterial({color:  0x002900});

// TERRAIN NOISE
var sharedPrefix = `
uniform sampler2D noiseTexture;
float getYPosition(vec2 p){
	return 120.0*(2.0*texture2D(noiseTexture, p/4800.0).r - 1.0);
}
`;
...
let x, y, z, w, angle, sinAngle, rotationAngle, rotationAxis;
...
function init() {
	...
	scene.fog = new THREE.Fog( "#87CEEB", 0, 1500 );
...
function animate() {
	...
	if ( controls.isLocked === true ) {
		...
		grassTime += delta*5;
		grassMaterial.uniforms.time.value = grassTime;
